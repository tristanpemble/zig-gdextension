//! This module defines the schema for the GDExtension API JSON file that describes all of the
//! methods and types for working with GDExtensions.
...
pub fn parse(allocator: Allocator, contents: []const u8) !Parsed(Api) {
    return try std.json.parseFromSlice(Api, allocator, contents, .{
        .ignore_unknown_fields = false,
    });
}

pub fn parseLeaky(allocator: Allocator, contents: []const u8) !Api {
    return try std.json.parseFromSliceLeaky(Api, allocator, contents, .{
        .ignore_unknown_fields = false,
    });
}
